...
import {
  ...
  IMyStockInfo,
  ISearchResult,
} from "../types/stock/service";

import { getDay } from "../util/date";

export const getInterestStockInfoService = async (
  ticker: string,
  period: string
) => {
  let result: IInterestStockInfo = {
    ticker,
    price: 0,
    rate: 0,
    history: [],
  ...
  try {
    ...
    const historyData = await yahooFinance.historical(
      `${ticker}`,
      queryOptions
    );
...
export const getSearchTickerService = async (ticker: string) => {
  ...
  try {
    const { shortName, market, symbol } = await yahooFinance.quote(ticker);
    if (
      (ticker.includes(".KQ") || ticker.includes(".KS")) &&
      market != "kr_market"
    ) {
      result.isError = true;
    } else {
